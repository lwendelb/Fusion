...
def nlDRAM(paramList, init_z, lower, upper, y_x=None, y_n=None, x=None, L=20, shrinkage=0.2, s_p=(2.4**2), epsilon=1e-4, m0=0, sd0=1, c_y=0.1, d_y=0.1, c_g=0.1, d_g=0.1, c_b=0.1, d_b=0.1, adapt=20, thin=1, iters=5000, burn=2000, update=500, plot=True, fix=False):
    ...
    y_n_sm = lowess(endog=y_n, exog=x, frac=12.0/len(x), return_sorted=False)
    ...
    tau_y_x = 4.                                          # Error variance for Y
    tau_y_n = 2.
    ...
    var_scale_x = np.zeros_like(x)+1e-10+np.abs(y_x_sm)/np.max(np.abs(y_x_sm))                            # Scale for y_sm/tau_y
    var_scale_n = np.zeros_like(x)+1e-10+np.abs(y_n_sm)/np.max(np.abs(y_n_sm))

    plt.figure(1)
    plt.plot(Calc.x,Calc.ysin_mean)
    plt.plot(Calc.x,y_x_sm,'r+')
    ...
    att = att_S2 = acc_S1 = acc_S2 = att_2 = acc_2 = 0   # Attempts / acceptances counters
    acc_burn = att_burn = 0
    ...
    for i in range(iters):
        ...
        ## Update Metropolis std deviation during burn-in
        if (0 < i <= burn) & (i % 25 is 0) & (att_2 > 0):
            if acc_2/att_2 < 0.3:
                shrinkage *= 0.8
            elif acc_2/att_2 > 0.6:
                shrinkage *= 1.2
            acc_2 = att_2 = 0 # reset counters for current stage 2 acceptance
        ...
        # See if burn-in is over
        if (i == burn):
            if (0.3>acc_S2/att_S2) or (acc_S2/att_S2>0.5):
                burn +=100
            else:
                acc_S2 = att_S2 = 0
                acc = att = 0
